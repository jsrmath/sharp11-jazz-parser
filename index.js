...
var parseChord = function (str) {
  return s11.chord.create(handleChordAliases(str));
};
...
var parseDuration = function (str, jazz) {
  ...
  if (jazz.info.time.toString() === '5,4' && str === '1') {
    return s11.duration.beats(5);
  }

  // If we're in 6/8 and we see a 2., treat it as 2 beats
  if (jazz.info.time.toString() === '6,8' && str === '2.') {
    return s11.duration.beats(2);
  }

  // Otherwise, use this map
  return {
    '8': s11.duration.subunit('eighth'), // TODO: Handle long vs short eighths
    '4': s11.duration.beats(1),
    '4.': s11.duration.beats(1).addSubunit('eighth'),
    '2': s11.duration.beats(2),
    '2.': s11.duration.beats(3),
    '1': s11.duration.beats(4),
    '1.': s11.duration.beats(6)
  }[str];
};
...
var parseSectionHeader = function (jazz, lines) {
  ...
  if (!jazz.info.key && (line = _.find(lines, function (l) {
    ...
    matches = line.match(/^\*(.+):$/);
    jazz.info.key = s11.note.create(handleChordAliases(matches[1])).name;
...
var parseJazzString = function (str) {
  ...
  return s11.chart.create(jazz.sections, jazz.content, jazz.info);
};
